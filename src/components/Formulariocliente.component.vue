...
const formularioClientevarios = async () => {
  try {
    ...
    await axios.post("https://104.248.115.32:8080/api/clientes", clientePayload, {
    ...
    const response = await axios.get("https://104.248.115.32:8080/api/clientes/ultimo");
    ...
    if (form.value.estadoCivil === 2) {
      ...
      await axios.post("https://104.248.115.32:8080/api/clienteConyuges", conyugePayload, {
        withCredentials: true,
        headers: {
          "Content-Type": "application/json",
          "X-User-ID": idOperario,
  ...
  try {
    ...
    if (form.value.numCopropietarios > 0) {
      form.value.copropietarios.forEach((copropietario) => {
        ...
        requests.push(
            axios.post("https://104.248.115.32:8080/api/copropietario", copropietarioPayload, {
              withCredentials: true,
              headers: {
                "Content-Type": "application/json",
                "X-User-ID": idOperario,
        ...
        if (copropietario.estadoCivilCopropietarios === 2) {
          ...
          requests.push(
              axios.post("https://104.248.115.32:8080/api/copropietarioconyuge", copropietarioconyugePayload, {
                withCredentials: true,
                headers: {
                  "Content-Type": "application/json",
                  "X-User-ID": idOperario,
    ...
    if (form.value.numLotes > 0) {
      for (const lote of form.value.lotes) {
        ...
        await axios.post("https://104.248.115.32:8080/api/lotes", lotePayload, {
          withCredentials: true,
          headers: {
            "Content-Type": "application/json",
            "X-User-ID": idOperario,
        ...
        const { data: idLote } = await axios.get("https://104.248.115.32:8080/api/lotes/ultimo");
...
const finalizarRegistroDetalleLote = async () => {
  try {
    const requests = form.value.lotes.map(lote => {
      ...
      return axios.post('https://104.248.115.32:8080/api/detallesLote', detallesLoteDTO);
    });
